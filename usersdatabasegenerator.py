...
conn.execute('''CREATE TABLE User_Auth(UserId text PRIMARY KEY, Password text not null)''')

#creating user information database (after successfull authentication):

conn.execute('''CREATE TABLE User_Info(UserId text PRIMARY KEY, Password text not null, Reviews text not null, Ratings real)''')
